...
from .config import APP, DIST_ROOT, PACKAGE_NAME
...
def package_prototype(prototype: Path) -> Path:
    ...
    for entry in entries:
        ...
        if path.is_absolute() or ".." in path.parts or path.parts[0] != prototype.name:
            raise RuntimeError(f"发行包包含不安全路径: {entry}")
    required = {
        f"{prototype.name}/ChatGPT",
        f"{prototype.name}/run-chatgpt-linux.sh",
        f"{prototype.name}/resources/app.asar",
    }
